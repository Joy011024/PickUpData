...
namespace TimeIntervalListen
{
    /// <summary>
    /// 调用该程序集的对象 必备程序集  Common.Logging, Version=3.3.1.0 Common.Logging.Core, Version=3.3.1.0
    /// </summary>
    /// <param name="obj"></param>
    public delegate void BaseDelegate(object obj);
    public class QuartzJob
    {
        ...
        public QuartzJob(string groupstr) 
        {
            group = groupstr;
        }
        ...
        {
            IScheduler sch = GetScheduler();
            List<IJobExecutionContext> jobs= sch.GetCurrentlyExecutingJobs().ToList();//获取全部活跃的作业列表
            List<string> gs= sch.GetJobGroupNames().ToList();
        ...
        public void CreateJob<T>(int interval, int repeactCount) where T : IJob //调用的实体需要继承IJob
        {
            ...
            {
                job = JobBuilder.Create<T>().WithIdentity(jobName, group).Build();//job
                jy = new JobKey(jobName, group);
            }
            else 
            {
                job = JobBuilder.Create<T>().WithIdentity(jobName).Build();
                jy = new JobKey(jobName);
            }
        ...
        /// <param name="param">object 对象的数组【元素1 轮询执行触发的事件，元素2 触发事件的参数列表】</param>
